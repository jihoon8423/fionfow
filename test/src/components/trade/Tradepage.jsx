...
function Tradepage(){
  ...
  const handleTradeTypeChange = (type) => {
    ...
    const getPlayerPrice = (id, grade) => {
      const player = data.find(item => item.id === id);
      if (player) {
        const priceKey = `str${grade}`;
        return player[priceKey] || 'Unknown';
      }
      return 'Unknown';
    };
         ...
         <div className='trade-result-header'>거래 기록</div>
          <div className='trade-result'>
            {tradeRecords.map(record => {
              ...
              const price = getPlayerPrice(record.spid, record.grade);

              const playerClass = player ? player.smallclass : 'Unknown';

              const diffprice = price - record.value;
              const diffpriceFormatted = diffprice.toLocaleString();
              const diffpriceWithSign = (diffprice > 0 ? "+" : "") + diffpriceFormatted;
              const diffRatio = (((price- record.value) / record.value) * 100).toFixed(2);
              ...
              return (
                <tr key={record.saleSn} className='trade-box'>
                  ...
                  <div className='trade-price'>거래가: {record.value.toLocaleString()}</div>
                  <div className='trade-marketprice'>현재시세: {price.toLocaleString()}</div>
                  <div className='trade-diffprice'>{diffpriceWithSign} ({diffRatio > 0 ? "+" : "-"}{Math.abs(diffRatio)}%)</div>
                </tr>
              );
            })}
          </div>
